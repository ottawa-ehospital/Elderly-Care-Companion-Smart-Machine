...
class PoseFallPipeline:
    def __init__(self, model_path=None):
        if model_path is None or str(model_path).strip() == "":
            model_path = os.path.join(os.getcwd(), "pose_landmarker_heavy.task")
    ...
    def process(self, frame_bgr, ts_ms: int):
        ...
        # ✅ 先 clamp 到 0..1 再算 bbox（避免 bh>1、cx 飘）
        ...
        if show_posture:
            cv2.rectangle(overlay, (x1, max(0, y1 - 34)), (x1 + 240, y1), (0, 0, 0), -1)
            cv2.putText(
                overlay, posture, (x1 + 8, y1 - 10),
                cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 255), 2, cv2.LINE_AA
            )

        person_cx = _clamp01((x_min + x_max) / 2.0)
        person_bh = _clamp01(max(1e-6, (y_max - y_min)))  # 0..1
